...
class IpfsMultiplexer {
  constructor(options = {}) {
    ...
    this.ipfsClients = _gateways.default.map(gateway => (0, _ipfsHttpClient.default)({ ...gateway,
      ...this.options
    }));
  }
  ...
  addGateway(gateway) {
    ...
    this.ipfsClients.push((0, _ipfsHttpClient.default)({ ...this.options,
      ...gateway
    }));
  }
...
const createFunction = (method, optionsIndex) => {
  return function (...args) {
    ...
    if (optionsIndex !== null && args[optionsIndex].signal === undefined) // eslint-disable-next-line no-param-reassign
      args[optionsIndex].signal = abortController.signal;
    return new Promise((resolve, reject) => {
      let rejected = 0;
      ipfsClients.forEach(ipfsClient => {
        (0, _selector.default)(ipfsClient, methodSelector)(...args).then(response => {
          abortController.abort();
          resolve(response);
        }).catch(() => {
          rejected += 1;
          if (rejected === ipfsClients.length) reject(new Error('The requests from all nodes failed'));
        });
      });
    });
  };
...
Object.entries(_ipfsClientMethods.default).forEach(([method, optionsIndex]) => {
...
var _default = IpfsMultiplexer; // const ipfsMultiplexer = new IpfsMultiplexer();
// const date = new Date();
// ipfsMultiplexer
